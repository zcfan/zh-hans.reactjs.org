...
一些组员还做了[编辑器集成配置](https://prettier.io/docs/en/editors.html)。Prettier 给我们的使用体验非常好，我们也推荐任何编写 JavaScript 的团队使用它。

## 重新组织 Monorepo {#restructuring-the-monorepo}

自从 React 拆分成多个包之后，它就变成了一个 [monorepo](https://danluu.com/monorepo/)：被组织在同一个代码仓库下的一组包。这使得同步改变和共享工具链简单了，但我们的文件夹结构以前深层嵌套难以理解。很难看出哪些文件归属于哪些包。在发布 React 16 之后，我们决定要完全彻底重新组织仓库结构。以下是我们如何做到这点的。

### 迁移到 Yarn Workspace {#migrating-to-yarn-workspaces}

Yarn 包管理器在几个月前[引入了一个叫做 Workspace 的特性](https://yarnpkg.com/blog/2017/08/02/introducing-workspaces/)。这个特性让你可以告诉 Yarn 你的 monorepo 里的包都位于哪里。每次你运行 `yarn` 时，在安装你的依赖以外，它还会设置好从你项目的 `node_modules` 到你的所有包的源码文件夹的符号链接。

多亏了 Workspace，在我们自己的多个包间进行 absolute import （比如在 `react-dom` 中 import `react`) 在任何支持 Node 解析机制的工具中都无需额外措施即可工作。我们遇到的唯一的问题是 Jest 不会在符号链接的包中运行 transform ，但也找到了[解决方案](https://github.com/facebook/jest/pull/4761)并被 Jest 合并。

要启用 Yarn Workspace，我们需要向我们的 [`package.json`](https://github.com/facebook/react/blob/cc52e06b490e0dc2482b345aa5d0d65fae931095/package.json#L4-L6) 中添加 `"workspaces": ["packages/*"]` ，并将所有代码和它们的 `package.json` 文件移至[顶层的 `packages/*` 文件夹](https://github.com/facebook/react/tree/cc52e06b490e0dc2482b345aa5d0d65fae931095/packages)

每个包都以相似的方式组织。对于每个公开 API 入口，比如 `react-dom` 或 `react-dom/server` ，都会在包的根文件夹有这样一个[文件](https://github.com/facebook/react/blob/cc52e06b490e0dc2482b345aa5d0d65fae931095/packages/react-dom/index.js)。它会把 [`/src/`](https://github.com/facebook/react/tree/cc52e06b490e0dc2482b345aa5d0d65fae931095/packages/react-dom/src) 子文件夹中的实现重新导出。我们是刻意将入口指向源码而非预编译版本的。通常来说我们在开发期间每次修改后都会重新运行一部分测试子集。每次运行测试都要编译一遍项目会让这个过程慢的无法忍受。但当我们要把包发布到 npm 时，我们会将这些入口文件替换为 [`/npm/`](https://github.com/facebook/react/tree/cc52e06b490e0dc2482b345aa5d0d65fae931095/packages/react-dom/npm) 文件夹中对应的版本，它们将会指向到编译后的成品。

不是所有的包都必须要发布到 npm 。比如，我们将一些足够小、且能够安全的被复制到多处地方的工具函数放在一个叫做 [`shared` 的伪包（pseudo-package）](https://github.com/facebook/react/tree/cc52e06b490e0dc2482b345aa5d0d65fae931095/packages/shared)中。我们的打包器被配置成[只将 `package.json` 中的 `dependencies` 声明认作外部的](https://github.com/facebook/react/blob/cc52e06b490e0dc2482b345aa5d0d65fae931095/scripts/rollup/build.js#L326-L329)。所以它会把 `shared` 中的代码打包进 `react` 和 `react-dom` 中，不会在打包结果中留下任何指向 `shared/` 的饮用。因此即使你不打算发布 npm 包也完全可以使用 Yarn Workspace 。
